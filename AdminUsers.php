  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="row">
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <button type="button" id="eliminarUser" onclick="eliminarUser(<?= $usuario['id'] ?>)" class="btn btn-primary">Eliminar
                            User</button>
                        </td>
                        <td>
                        <!--  <button type="button" id="desactivarUser" onclick="desactivarUser(<?= $usuario['id'] ?>,<?= $usuario['is_active'] ?>)" class="btn btn-primary">Banear User</button>-->
                        </td>
                        <td><a class="btn btn-primary" href="AdminUsers.php?user_id=<?= $usuario['id'] ?>">
                            Seleccionar</a></td>
                      </tr>
                    <?php } ?>
                  </tbody>
        ...
        <div class="col-sm-6">
          <h2>Datos Usuario seleccionado</h2>
          <?php if (empty($_GET['user_id'])) {
          } else { ?>
            <?php try {
              $pdo = $db->connect();

              $query_2 = "SELECT * FROM user WHERE role_id = 1 and id = " . $_GET['user_id'];
              ...
              foreach ($usuarios_2 as $usuario_2) { ?>
                <form action="../../CulturalCompassBackEnd/Controladores/AccionesUserOrg.php" method="POST">
                  ...
                  <button type="submit" class="btn btn-primary">Guardar Cambios Usuario</button>
                </form>
          <?php }
            } catch (PDOException $e) {
              echo "Error: " . $e->getMessage();
        ...
        <div class="row">
          ...
          <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
              <?php 
              $mas = $base_de_datos->query
              ("SELECT orden.id as orden, evento.name_event as name_event, orden.total_price as total_price, clientes.pago as metodo, orden.created as fecha 
          ...
          <div class="col-sm-6">
            <h2>Historial de asistencia eventos</h2>
            <?php if (!empty($_GET['user_id'])) { ?>
              <?php
              $queryEventos = "SELECT * FROM event_users eu INNER JOIN evento e ON e.id = eu.id_event WHERE eu.id_user  = " . $_GET['user_id'];
      ...
      <script src="./../js/AdminUsers.js" defer></script>
  </body>
